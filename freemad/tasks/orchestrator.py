...
class TaskOrchestrator:
    ...
    def step(self, task_id: str) -> TaskSnapshot:
        ...
        try:
            ...
            return self._fail(
                self._require_task(task_id), f"command timed out: {exc.cmd}"
            )
        except Exception as exc:  # pragma: no cover - defensive boundary
            return self._fail(
                self._require_task(task_id), f"autonomous task crashed: {exc}"
            )
    ...
    def _run_research(self, task: TaskSnapshot) -> TaskSnapshot:
        ...
        response = self._invoke_agent(
            proposer, task, TaskRole.RESEARCHER, TaskStage.RESEARCH
        )
        ...
        if response.sources:
            source_artifact = self._record_artifact(
                ...
                role=TaskRole.RESEARCHER,
                content=json.dumps(
                    [source.to_dict() for source in response.sources],
                    indent=2,
                    sort_keys=True,
                ),
                created_by_agent_id=proposer.agent_cfg.id,
        ...
        checker = self._select_agent_for_role(
            TaskRole.REVIEWER, exclude={proposer.agent_cfg.id}
        )
        if checker is None:
            checker = self._select_agent_for_role(
                TaskRole.RESEARCHER, exclude={proposer.agent_cfg.id}
            )
        if checker is None:
            return self._pause(task, "missing_research_checker")
        review = self._invoke_agent(
            checker, task, TaskRole.REVIEWER, TaskStage.RESEARCH
        )
    ...
    def _run_draft_plan(self, task: TaskSnapshot) -> TaskSnapshot:
        ...
        response = self._invoke_agent(
            proposer, task, TaskRole.PLANNER, TaskStage.DRAFT_PLAN
        )
        artifact = self._record_artifact(
            task,
        ...
        work_items = tuple(
            self._normalize_work_item(item, task.task_id)
    ...
    def _run_plan_review(self, task: TaskSnapshot) -> TaskSnapshot:
        ...
        proposer = self._select_agent_for_role(TaskRole.PLANNER)
        reviewer = self._select_agent_for_role(
            TaskRole.REVIEWER, exclude={proposer.agent_cfg.id} if proposer else set()
        )
        if reviewer is None:
            return self._pause(task, "missing_plan_reviewer")
        response = self._invoke_agent(
            reviewer, task, TaskRole.REVIEWER, TaskStage.PLAN_REVIEW
        )
        ...
        if decision == ReviewDecision.APPROVE:
            next_stage = (
                TaskStage.FINALIZE
                if updated.task_type == TaskType.PLAN
                else TaskStage.EXECUTE
            )
            return self._persist(
                replace(
                    updated, current_stage=next_stage, iteration=updated.iteration + 1
                )
            )
    ...
    def _run_execute(self, task: TaskSnapshot) -> TaskSnapshot:
        ...
        for group in self._partition_work_items(work_items):
            ...
            if self.cfg.budget.max_concurrent_agents is not None:
                max_workers = max(
                    1, min(max_workers, self.cfg.budget.max_concurrent_agents)
                )
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=max_workers
            ) as executor:
                futures = [
                    executor.submit(self._execute_work_item, task, work_item)
    ...
    def _run_code_review(self, task: TaskSnapshot) -> TaskSnapshot:
        ...
        reviewer = self._select_agent_for_role(
            TaskRole.REVIEWER,
            exclude={implementer.agent_cfg.id} if implementer else set(),
        )
        if reviewer is None:
            return self._pause(task, "missing_code_reviewer")
        response = self._invoke_agent(
            reviewer, task, TaskRole.REVIEWER, TaskStage.CODE_REVIEW
        )
        decision = response.review_decision or ReviewDecision.REJECT
        review_artifact = self._record_artifact(
            task,
        ...
        updated = self._append_attempt(
            ...
            StageAttempt(
                ...
                proposer_agent_id=implementer.agent_cfg.id
    ...
    def _run_verify(self, task: TaskSnapshot) -> TaskSnapshot:
        ...
        implementer = self._select_agent_for_role(TaskRole.IMPLEMENTER)
        verifier = self._select_agent_for_role(
            TaskRole.VERIFIER,
            exclude={implementer.agent_cfg.id} if implementer else set(),
        )
        if verifier is None:
        ...
        response = self._invoke_agent(
            verifier, task, TaskRole.VERIFIER, TaskStage.VERIFY
        )
        decision = response.review_decision or ReviewDecision.REJECT
    ...
    def _run_finalize(self, task: TaskSnapshot) -> TaskSnapshot:
        ...
        self._record_artifact(
            ...
            role=TaskRole.PLANNER,
            content=content,
            created_by_agent_id=self._select_agent_for_role(
                TaskRole.PLANNER
            ).agent_cfg.id,
    ...
    def _invoke_agent(
        ...
        *,
        ...
        if not isinstance(response, TaskResponse):
            raise ConfigError(
                f"agent {agent.agent_cfg.id} returned unsupported autonomous response type"
            )
        return response
    ...
    def _select_agent_for_role(self, role: TaskRole, *, exclude: Iterable[str] = ()):
        ...
        for agent_cfg in self.cfg.agents:
            if (
                not agent_cfg.enabled
                or agent_cfg.id in excluded
                or agent_cfg.id not in self.agents
            ):
                continue
    ...
    def _resolve_review_dispute(
        ...
        *,
        ...
        if attempts <= self.cfg.task.max_stage_retries:
            return self._persist(
                replace(task, current_stage=retry_stage, iteration=task.iteration + 1)
            )

        arbiter = self._select_agent_for_role(
            TaskRole.ARBITER, exclude=(proposer_agent_id, reviewer_agent_id)
        )
        if arbiter is None:
        ...
        return self._wait_for_human(
            updated, response.findings[0] if response.findings else response.content
        )

    def _append_attempt(
        self, task: TaskSnapshot, attempt: StageAttempt
    ) -> TaskSnapshot:
    ...
    def _attempt_count(self, task: TaskSnapshot, stage: TaskStage) -> int:
        return len(
            [attempt for attempt in task.stage_attempts if attempt.stage == stage]
        )
    ...
    def _apply_writes(
        self, task: TaskSnapshot, writes: Sequence[FileWrite], work_item: WorkItem
    ) -> None:
        ...
        for write in writes:
            ...
            if not self._is_under_roots(
                target, workspace_root, self.cfg.task.tool_policy.allowed_write_roots
            ):
                raise ConfigError(f"write path outside allowed roots: {write.path}")
            if work_item.write_scope and not self._is_under_roots(
                target, workspace_root, work_item.write_scope
            ):
    ...
    def _run_command(
        self, task: TaskSnapshot, command: str, *, stage: TaskStage
        ...
        content, _ = enforce_size(
            content, self.cfg.security.max_solution_size, label="command_log"
        )
    ...
    def _partition_work_items(
        self, work_items: Sequence[WorkItem]
    ) -> List[List[WorkItem]]:
        ...
        for work_item in work_items:
            ...
            for group in groups:
                existing_scope = {
                    path for existing in group for path in existing.write_scope
                }
    ...
    def _emit_stage_started(
        self, task: TaskSnapshot, stage: TaskStage, role: TaskRole
    ) -> None:
        self._emit(
            TaskEvent(
    ...
    def _wait_for_human(self, task: TaskSnapshot, reason: str) -> TaskSnapshot:
        waiting = self._persist(
            replace(task, status=TaskStatus.WAITING_FOR_HUMAN, error=reason)
        )
        self._emit(
            TaskEvent(
    ...
    def _is_under_roots(
        self, target: Path, workspace_root: Path, roots: Iterable[str]
    ) -> bool:
        for root in roots:
