...
class ScoreTracker:
    ...
    def record_initial(
        ...
        self._add(
            ...
            round_idx=round_idx,
            agent_id=agent_id,
            action=ScoreAction.INITIAL.value,
        )
    ...
    def record_keep(self, *, agent_id: str, answer_id: str, round_idx: int) -> None:
        ...
        self._add(
            ...
            round_idx=round_idx,
            agent_id=agent_id,
            action=ScoreAction.KEEP.value,
        )

    def record_change(
        self, *, agent_id: str, old_answer_id: str, new_answer_id: str, round_idx: int
    ) -> None:
        f = self._decay(round_idx)
        ...
        self._add(
            ...
            round_idx=round_idx,
            agent_id=agent_id,
            action=ScoreAction.CHANGE.value,
        )
    ...
    def get_all_scores(self) -> Dict[str, float]:
        ...
        for ans, raw in self._raw.items():
            c = (
                max(1, len(self._contributors.get(ans, set())))
                if self.cfg.scoring.normalize
                else 1
            )
            out[ans] = raw / c
        return out
