...
def _parse_sections(raw: str, markers: List[str]) -> Dict[str, str]:
    ...
    """
    pattern = re.compile(
        _HEADER_RE_FMT % ("|".join(markers)), re.IGNORECASE | re.MULTILINE
    )
...
def parse_generation(raw: str) -> ParseResultGen:
    ...
    if not raw or not raw.strip():
        return ParseResultGen(
            solution="", reasoning="", needs_retry=True, errors=["empty output"]
        )
    sections = _parse_sections(
        raw, [GenMarker.SOLUTION.value, GenMarker.REASONING.value]
    )
    ...
    return ParseResultGen(
        solution=solution, reasoning=reasoning, needs_retry=bool(errors), errors=errors
    )
...
def parse_critique(raw: str) -> ParseResultCrit:
    ...
    sections = _parse_sections(
        raw,
        [
            CritMarker.DECISION.value,
            CritMarker.REVISED_SOLUTION.value,
            CritMarker.REASONING.value,
        ],
    )
    if (
        CritMarker.DECISION.value not in sections
        or not sections.get(CritMarker.DECISION.value, "").strip()
    ):
    ...
    if (
        CritMarker.REVISED_SOLUTION.value not in sections
        or not sections[CritMarker.REVISED_SOLUTION.value].strip()
    ):
    ...
    return ParseResultCrit(
        decision=Decision.REVISE,
        solution=sections[CritMarker.REVISED_SOLUTION.value].strip(),
        reasoning=reasoning,
        needs_retry=False,
        errors=[],
    )
